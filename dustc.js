...
define(['./dust'], function (dust) {
	...
	// Function which parses module name
	...
	return {
		load: function (name, req, load, config) {
			var parsed = parseModuleName(name);
			// Check to see if we have already compiled the template
			if (buildCache[name]) {
			...
			(function (done) {
					...
					xhr.onreadystatechange = function () {
						if (xhr.readyState === 4) {
							if (xhr.status < 399) {
								...
								throw 'Could not retrieve template: ' + parsed.file;
							}
						}
					};
					xhr.send(null);
				...
				// Check again to see if we have already compiled the template in order to prevent race conditions
				if (buildCache[name]) {
					return load(parsed.name);
				}
				buildCache[name] = dust.compile(text, parsed.name);
				dust.loadSource(buildCache[name]);
				load(parsed.name);
			});
		},
		write: function (pName, mName, write) {
			if (buildCache[mName]) {
				var parsed = parseModuleName(mName);
				write.asModule(pName + '!' + mName, 'define(["dust"], function (dust) { ' + buildCache[mName] + '; return "' + parsed.name + '"; });');
			}
		}
	};
});